...
class client(object):
    ...
    def start_signal(self, function_code=0, next_buf_size=0):
        ...
        buf = ((function_code<<4) + (next_buf_size)).to_bytes(1, 'big', False)
        self.send(buf)
    ...
    def send_long_data(self, buf_len_code=1, data=b'abcd'):
        ...
        while True:
            print('page:',page)
            recv_buf = int.from_bytes(self.recv(buf_len, 2), 'big', False)
            #判断同步指令
            ...
            send_buf += last_byte.to_bytes(1, 'big', False)
            self.send(send_buf)
            page+=1
        ...
        while working:
            ...
            if last_byte_int & 0b00000001:
                ...
                if page==remote_page:
                    ...
                    #同步指令
                    ack = (page<<1).to_bytes(1, 'big', False)
...
class e_paper(client):
    ...
    def BW_pic(self, buf_size=2, file_name = '1.jpg'):
    ...
    def RGB_pic(self, buf_size=2, file_name = '1.jpg', size=(8,8)):
        self.start_signal(5,buf_size)
        self.send(size[0].to_bytes(2, 'big', False))
        self.recv(buf_len=1)
        self.send(size[1].to_bytes(2, 'big', False))
        self.recv(buf_len=1)
        self.send_long_data(buf_size, file_name.encode())
        self.receive_tomeout=10
        data = self.receive_long_data()
...
if __name__ == '__main__':
    a=e_paper()
    a.connect("172.0.0.1", 8266)
    #a.test()
    #a.test1(buf_size=7)
    #a.BW_pic(buf_size=5, file_name = '4.jpg')
    a.RGB_pic(buf_size=5, file_name = '4.jpg')
